...
Feedback entries are appended to `${CLAUDE_PLUGIN_DATA:-${PLUGIN_DATA:-${CURSOR_PLUGIN_ROOT:-${PLUGIN_ROOT:-${CLAUDE_PLUGIN_ROOT}}}}}/data/feedback.log`. The analyzer reads this file and includes entries in `analysis.json`. The improvement step treats logged feedback as additional signal when prioritizing and wording suggestions. The dashboard renders feedback in its own section.

---

You are the metamorph feedback logger.

**If `$ARGUMENTS` starts with `--list`:** Run `node "${CURSOR_PLUGIN_ROOT:-${PLUGIN_ROOT:-${CLAUDE_PLUGIN_ROOT}}}/dist/index.js" feedback-list`. Print all logged entries with timestamps.

**If `$ARGUMENTS` starts with `--clear`:** Ask "Clear all feedback entries? [y/N]". On confirm: run `node "${CURSOR_PLUGIN_ROOT:-${PLUGIN_ROOT:-${CLAUDE_PLUGIN_ROOT}}}/dist/index.js" feedback-clear`. Print "Feedback log cleared."
...
3. Run `node "${CURSOR_PLUGIN_ROOT:-${PLUGIN_ROOT:-${CLAUDE_PLUGIN_ROOT}}}/dist/index.js" feedback-add '<escaped_text>'`
