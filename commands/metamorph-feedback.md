...
**If `$ARGUMENTS` starts with `--list`:** Run `node "${CLAUDE_PLUGIN_ROOT}/dist/index.js" feedback-list`. Print all logged entries with timestamps.

**If `$ARGUMENTS` starts with `--clear`:** Ask "Clear all feedback entries? [y/N]". On confirm: run `node "${CLAUDE_PLUGIN_ROOT}/dist/index.js" feedback-clear`. Print "Feedback log cleared."
...
3. Run `node "${CLAUDE_PLUGIN_ROOT}/dist/index.js" feedback-add '<escaped_text>'`
