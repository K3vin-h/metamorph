import type { MistakeEvent, MistakePattern, SessionProfile } from "../types.js";
import { readPersistedMistakeEvents } from "../mistakeFeedback.js";

const MAX_EXAMPLES_PER_PATTERN = 2;
const MAX_PATTERNS_PER_TARGET = 4;
const MAX_EXAMPLE_CHARS = 80;

function trim(s: string): string {
  const t = s.replace(/\s+/g, " ").trim();
  return t.length <= MAX_EXAMPLE_CHARS ? t : t.slice(0, MAX_EXAMPLE_CHARS - 3) + "...";
}

function exampleKey(tool: string, mistake: string, correction?: string): string {
  return `${tool}::${mistake}::${correction ?? ""}`;
}

function collectEvents(
  sessions: Record<string, SessionProfile>,
  pluginRoot: string
): MistakeEvent[] {
  const fromSessions: MistakeEvent[] = [];
  for (const session of Object.values(sessions)) {
    fromSessions.push(...(session.mistakeEvents ?? []));
  }
  return [...fromSessions, ...readPersistedMistakeEvents(pluginRoot)];
}

export function aggregateMistakePatterns(
  sessions: Record<string, SessionProfile>,
  pluginRoot: string,
  targetId: string,
  targetKind: "agent" | "skill"
): MistakePattern[] {
  const byKey = new Map<
    string,
    MistakePattern & { _examples: Map<string, { mistake: string; correction?: string }> }
  >();

  for (const ev of collectEvents(sessions, pluginRoot)) {
    if (ev.targetKind !== targetKind || ev.targetId !== targetId) continue;

    const tool = ev.toolName || "any";
    const key = `${ev.kind}:${tool}`;

    if (!byKey.has(key)) {
      byKey.set(key, {
        kind: ev.kind,
        tool,
        count: 0,
        examples: [],
        _examples: new Map(),
      });
    }

    const bucket = byKey.get(key)!;
    bucket.count++;

    const exKey = exampleKey(tool, ev.mistakeSummary, ev.correctionSummary);
    if (!bucket._examples.has(exKey) && bucket._examples.size < MAX_EXAMPLES_PER_PATTERN) {
      bucket._examples.set(exKey, {
        mistake: trim(ev.mistakeSummary),
        ...(ev.correctionSummary ? { correction: trim(ev.correctionSummary) } : {}),
      });
    }
  }

  const patterns: MistakePattern[] = [];
  for (const bucket of byKey.values()) {
    patterns.push({
      kind: bucket.kind,
      tool: bucket.tool,
      count: bucket.count,
      examples: [...bucket._examples.values()],
    });
  }

  patterns.sort((a, b) => b.count - a.count);
  return patterns.slice(0, MAX_PATTERNS_PER_TARGET);
}
